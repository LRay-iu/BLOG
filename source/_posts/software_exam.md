...
STP，在网络出现二层环路时，通过特定的逻辑阻塞打破环路，并在网络出现拓扑变更时及时收敛，保障网络冗余性

#### 网桥

桥ID一共由8个字节组成，2个字节优先级和6个字节的MAC地址组成

桥优先级**默认为32768**，可以手工修改（越小越优先，一般是4096的倍数）
...
<img src="software_exam/image-20241007212926379.png" alt="image-20241007212926379" style="zoom: 67%;" />

如果一个链路上有多个端口具有相同的到跟的开销，则进一步比较接口上收到的BPDU优劣：比较交换机的MAC地址，接口地址大小（越小越优先）。

#### 生成树协议

**生成树协议STP**【802.1D】：慢，收敛要30-50s

**快速生成树协议RSTP**【802.1W】：快，6s内完成网络拓扑快速收敛

**多生成树协议MSTP**【802.1S】：实现多个VLAN数据的负载均衡
...
- IEEE802.1ad 打了两层VLAN标签，将用户VLAN嵌套在运营商城运以太VLAN中传送
- IEEE802.1ah 也称PBB，也叫MAC-IN-MAC技术
